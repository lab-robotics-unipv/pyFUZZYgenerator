...
int newState_roll(fuzzyLogic_roll * fl) {
	...
	memFunction_roll MF_diff[3];

	double poi_diff_neg[] = { -5.0, -5.0, 0.0 };
	createMemFunction_roll(&(MF_diff[0]), "neg", TRI_MF, poi_diff_neg);
	double poi_diff_zero[] = { -5.0, 0.0, 5.0 };
	createMemFunction_roll(&(MF_diff[1]), "zero", TRI_MF, poi_diff_zero);
	double poi_diff_pos[] = { 0.0, 5.0, 5.0 };
	createMemFunction_roll(&(MF_diff[2]), "pos", TRI_MF, poi_diff_pos);

	flag = createInput_roll(&(fi[0]), "diff", MF_diff, 3, -5.0, 5.0);
	if (flag < 0) {
		return 1;
	}
	...
	memFunction_roll MF_diffVel[3];

	double poi_diffVel_neg[] = { -1.0, -1.0, 0.0 };
	createMemFunction_roll(&(MF_diffVel[0]), "neg", TRI_MF, poi_diffVel_neg);
	double poi_diffVel_zero[] = { -1.0, 0.0, 1.0 };
	createMemFunction_roll(&(MF_diffVel[1]), "zero", TRI_MF, poi_diffVel_zero);
	double poi_diffVel_pos[] = { 0.0, 1.0, 1.0 };
	createMemFunction_roll(&(MF_diffVel[2]), "pos", TRI_MF, poi_diffVel_pos);

	flag = createInput_roll(&(fi[1]), "diffVel", MF_diffVel, 3, -1.0, 1.0);
	if (flag < 0) {
		return 1;
	}
