...
int createLogic_z_displ(fuzzyLogic_z_displ * fl, fuzzyInput_z_displ * inputs, fuzzyOutput_z_displ * outputs, rule_z_displ * r) {
	...
	memcpy(fl->fInput, inputs, N_INPUT_z_displ*sizeof(fuzzyInput_z_displ));
	memcpy(fl->fOutput, outputs, N_OUTPUT_z_displ*sizeof(fuzzyOutput_z_displ));

	int i, j, k;
	int index, index2;
	for (i = 0; i < N_RULES_z_displ; i++) {
		...
		for (j = 0; j < r[i].nIfRules; j++) {
			index = getInputFromName_z_displ(fl, r[i].ifRules[j].name);
			if (index<0) {
				return -1;
			} else {
				fl->rules[i].ifRulesIndex[j] = index;
			}
			index2 = getInputMFFromName_z_displ(&(fl->fInput[index]), r[i].ifRules[j].mf);
			if (index2 < 0) {
				return -1;
			} else {
		...
		for (j = 0; j < r[i].nThenRules; j++) {
			index = getOutputFromName_z_displ(fl, r[i].thenRules[j].name);
			if (index<0) {
				return -1;
			} else {
				fl->rules[i].thenRulesIndex[j] = index;
			}
			index2 = getOutputMFFromName_z_displ(&(fl->fOutput[index]), r[i].thenRules[j].mf);
			if (index2 < 0) {
				return -1;
			} else {
...
void fuzzify_z_displ(fuzzyLogic_z_displ * fl, double * inputs) {
	int i;
	for ( i = 0; i < N_INPUT_z_displ; i++) {
			getPercentageFromInput_z_displ( &(fl->fInput[i]), inputs[i], &(fl->fuzzified[i * MAX_N_MF_INPUT_z_displ]));
	}
}
...
int evaluateRules_z_displ(fuzzyLogic_z_displ * fl) {
	...
	for ( i = 0; i < N_OUTPUT_z_displ; i++) {
		for ( j = 0; j < fl->fOutput[i].nMF; j++) {
			fl->evaluated[i * MAX_N_MF_OUTPUT_z_displ + j] = 0;
		}
	}
	...
	for ( i = 0; i < N_RULES_z_displ; i++) {
		...
		builtRule_z_displ * r = &(fl->rules[i]);
		for ( j = 0; j < r->nIfRules; j++) {
			// TODO: possibility to choose the function
			minValue = MIN(minValue, fl->fuzzified[(r->ifRulesIndex[j]) * MAX_N_MF_INPUT_z_displ + r->ifRulesMFIndex[j]]);
		}
		...
		for ( j = 0; j < r->nThenRules; j++) {
			e = &(fl->evaluated[(r->thenRulesIndex[j]) * MAX_N_MF_OUTPUT_z_displ + r->thenRulesMFIndex[j]]);
...
int defuzzify_z_displ(fuzzyLogic_z_displ * fl, double * outputs) {
	...
	for ( i = 0; i < N_OUTPUT_z_displ; i++) {
		for ( j = 0; j < fl->fOutput[i].nMF; j++) {
			sum = sum + fl->evaluated[i * MAX_N_MF_OUTPUT_z_displ + j];
			aux = aux + (fl->fOutput[i].mf[j].poi[0]) * (fl->evaluated[i * MAX_N_MF_OUTPUT_z_displ + j]);
		}
		outputs[i] = aux/sum;
		aux = 0;
		sum = 0;
