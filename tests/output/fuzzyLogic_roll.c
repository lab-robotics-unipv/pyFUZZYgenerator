...
int createLogic_roll(fuzzyLogic_roll * fl, fuzzyInput_roll * inputs, fuzzyOutput_roll * outputs, rule_roll * r) {
	...
	memcpy(fl->fInput, inputs, N_INPUT_roll*sizeof(fuzzyInput_roll));
	memcpy(fl->fOutput, outputs, N_OUTPUT_roll*sizeof(fuzzyOutput_roll));

	int i, j, k;
	int index, index2;
	for (i = 0; i < N_RULES_roll; i++) {
		...
		for (j = 0; j < r[i].nIfRules; j++) {
			index = getInputFromName_roll(fl, r[i].ifRules[j].name);
			if (index<0) {
				return -1;
			} else {
				fl->rules[i].ifRulesIndex[j] = index;
			}
			index2 = getInputMFFromName_roll(&(fl->fInput[index]), r[i].ifRules[j].mf);
			if (index2 < 0) {
				return -1;
			} else {
		...
		for (j = 0; j < r[i].nThenRules; j++) {
			index = getOutputFromName_roll(fl, r[i].thenRules[j].name);
			if (index<0) {
				return -1;
			} else {
				fl->rules[i].thenRulesIndex[j] = index;
			}
			index2 = getOutputMFFromName_roll(&(fl->fOutput[index]), r[i].thenRules[j].mf);
			if (index2 < 0) {
				return -1;
			} else {
...
void fuzzify_roll(fuzzyLogic_roll * fl, double * inputs) {
	int i;
	for ( i = 0; i < N_INPUT_roll; i++) {
			getPercentageFromInput_roll( &(fl->fInput[i]), inputs[i], &(fl->fuzzified[i * MAX_N_MF_INPUT_roll]));
	}
}
...
int evaluateRules_roll(fuzzyLogic_roll * fl) {
	...
	for ( i = 0; i < N_OUTPUT_roll; i++) {
		for ( j = 0; j < fl->fOutput[i].nMF; j++) {
			fl->evaluated[i * MAX_N_MF_OUTPUT_roll + j] = 0;
		}
	}
	...
	for ( i = 0; i < N_RULES_roll; i++) {
		...
		builtRule_roll * r = &(fl->rules[i]);
		for ( j = 0; j < r->nIfRules; j++) {
			// TODO: possibility to choose the function
			minValue = MIN(minValue, fl->fuzzified[(r->ifRulesIndex[j]) * MAX_N_MF_INPUT_roll + r->ifRulesMFIndex[j]]);
		}
		...
		for ( j = 0; j < r->nThenRules; j++) {
			e = &(fl->evaluated[(r->thenRulesIndex[j]) * MAX_N_MF_OUTPUT_roll + r->thenRulesMFIndex[j]]);
...
int defuzzify_roll(fuzzyLogic_roll * fl, double * outputs) {
	...
	for ( i = 0; i < N_OUTPUT_roll; i++) {
		for ( j = 0; j < fl->fOutput[i].nMF; j++) {
			sum = sum + fl->evaluated[i * MAX_N_MF_OUTPUT_roll + j];
			aux = aux + (fl->fOutput[i].mf[j].poi[0]) * (fl->evaluated[i * MAX_N_MF_OUTPUT_roll + j]);
		}
		outputs[i] = aux/sum;
		aux = 0;
		sum = 0;
