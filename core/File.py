...
class Variable():
    def __init__(self, name, data):
        ...
        if tag_mf in data:
            self.mf = [MembershipFactory(f, data[tag_mf][f])
                       for f in data[tag_mf]]
            self.data = data
...
class Model():
    def __init__(self, name, data):
        ...
        self.description = data.get('description', '')
        if 'fuzzify' in data:
            self.input_var = [Variable(v, data['fuzzify'][v])
                              for v in data['fuzzify']]
        else:
            self.input_var = []
            if 'defuzzify' in data:
                self.output_var = [Variable(v, data['defuzzify'][v])
                                   for v in data['defuzzify']]
            else:
                self.output_var = []
