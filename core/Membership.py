...
class MembershipFunction(object):
    ...
    def type(self, value):
        if value != self.required_type:
            raise TypeError("{} type is '{}', not '{}'".format(
                self.printable_type_name, value, self.required_type))
        self.__type = value
    ...
    def check_parameters_scalar_order(self, param):
        ...
        if ordered != param:
            raise ValueError("{} parameters must sorted in increasing order (given '{}')".format(
                self.printable_type_name, str(param)))
...
class Singleton(MembershipFunction):
    def __init__(self, name, data):
        ...
        # set as one-element list to allow iteration on checking
...
class MembershipFactory():
    ...
    def getMF(self):
        ...
        if type_name == 'pairwise-linear':
            ...
            raise ValueError(
                "Invalid type {} in MF {}".format(type_name, self.name))
        return mf
