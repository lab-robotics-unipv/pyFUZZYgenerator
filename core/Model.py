        ...
        while len(self.rules) != numRules:
            ...
            for iv, mf in zip(self.input_var, ifMFs):
                ...
                for i, ov in enumerate(self.output_var):
                    r = next(
                        (rule for rule in iv.rules if rule['output'] == ov.name), None)
...
class ModelFSTD(Model):
    def __init__(self, data):
        ...
        if len(self.rules) != self.getCorrectNumRules():
            raise Exception("Correct number of rules for model {name} is {num}".format(
                name=self.name, num=self.getCorrectNumRules()))
